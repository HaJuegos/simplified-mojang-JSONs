...
interface AngelevelData extends BPComponent {
    /**
     * (Opcional) El nivel de ira que disminuira con el tiempo. Este reduce el nivel de ira hacia todas las molestias.
     * @type {number}
     */
    angerDecrementInterval?: number;

    /**
     * (Opcional) El aumento de ira que se aplica al umbral de ira cuando la entidad se enoja.
     * @type {number}
     */
    angryBoost?: number;

    /**
     * (Opcional) Umbral que define cuando se considera que la entidad esta enojada por una molestia.
     * @type {number}
     */
    angryThreshold?: number;

    /**
     * (Opcional) Nivel predeterminado de molestia para cualquier molestia en particular. Aqui se especifica cuanto se eleva el nivel de enojo ante cada provocacion.
     * @type {number}
     */
    defaultAnnoyingness?: number;

    /**
     * (Opcional) El nivel predeterminado de molestia que causan los proyectiles. Se especifica cuanto se eleva el nivel de enojo con cada provocacion.
     * @type {number}
     */
    defaultProjectileAnnoyingness?: number;

    /**
     * (Opcional) El nivel maximo de ira que se puede alcanzar. Se aplica a cualquier molestia.
     * @type {number}
     */
    maxAnger?: number;

    /**
     * (Opcional) Filtro de interferencias o de entidades que hara enojar la entidad.
     * @type {EntityFilter}
     */
    nuisanceFilter?: EntityFilter;

    /**
     * (Opcional) Lista de sonidos con condiciones que se ejecutan por nivel de enojo.
     * @type {?IncreaseEntitySounds[]}
     */
    onIncreaseSounds?: IncreaseEntitySounds[];

    /**
     * (Opcional) Define si la entidad debe dejar de tener como target a un enemigo si este cae por debajo del umbral de enojo.
     * @type {boolean}
     */
